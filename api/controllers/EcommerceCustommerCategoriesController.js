module.exports = {
	datatable: async function (req, res) {
		...
		var data = await EcommerceCustommerCategories.find({where: where, limit: limit, skip: skip, sort: sort});
		var recordsTotal = await EcommerceCustommerCategories.count({where: where});
