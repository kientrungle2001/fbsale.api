module.exports = {
	datatable: async function (req, res) {
		...
		where.or = [{name : {like: '%'+keywork+'%'}}, {color : {like: '%'+keywork+'%'}}];
		var sort = columns[order.column].data + ' ' + order.dir;
		var data = await CoreLabels.find({where: where, limit: limit, skip: skip, sort: sort});
		var recordsTotal = await CoreLabels.count({where: where});
