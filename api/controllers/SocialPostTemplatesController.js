module.exports = {
	datatable: async function (req, res) {
		...
		where.or = [{name : {like: '%'+keywork+'%'}}, {type : {like: '%'+keywork+'%'}}];
		var sort = columns[order.column].data + ' ' + order.dir;
		var data = await SocialPostTemplates.find({where: where, limit: limit, skip: skip, sort: sort});
		var recordsTotal = await SocialPostTemplates.count({where: where});
