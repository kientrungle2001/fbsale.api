module.exports = {
	datatable: async function (req, res) {
		...
		where.or = [{name : {like: '%'+keywork+'%'}}, {id : {like: '%'+keywork+'%'}}];
		var sort = columns[order.column].data + ' ' + order.dir;
		var data = await EcommerceCustommers.find({where: where, limit: limit, skip: skip, sort: sort});
		var recordsTotal = await EcommerceCustommers.count({where: where});
